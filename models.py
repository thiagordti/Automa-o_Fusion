...
class AutomacaoFusion:
    ...
    def cob_nv(self):
        for linha in range(self.linha_atual, len(self.planilha)):
            ...
            enviarkey_elemento(self.navegador,'id_tipoDeMedicao__',By.ID,'Variavel')# Tipo de medição - Padrão
            ...
            enviarkey_elemento(self.navegador,'var_dadosDaCobranca__dadosParaHistorico__parcelaContrato__',By.ID,'0') # Envia 0 para Parcela de Contrato
            enviarkey_elemento(self.navegador, 'var_dadosDaCobranca__dadosParaHistorico__inicioPrestacao__', By.NAME, pd.to_datetime(self.planilha.iloc[linha]['DATA_INICIO']).strftime('%d/%m/%Y')) # Envia Data Inicio            
            enviarkey_elemento(self.navegador, 'var_dadosDaCobranca__dadosParaHistorico__finPrestacao__', By.NAME, pd.to_datetime(self.planilha.iloc[linha]['DATA_FIM']).strftime('%d/%m/%Y')) # Envia Data Fim
            enviarkey_elemento(self.navegador,'id_dadosDaCobranca__dadosParaHistorico__diaLimiteNFCliente__',By.NAME , int(self.planilha.iloc[linha]['DIA LIMITE'])) # Envia Dia Limite NF Cliente
            enviarkey_elemento(self.navegador,'var_dadosDaCobranca__cobrancaRelacionadaComConvenio__',By.ID,'Não') # Envia Não para Cobrança Relacionada com Convênio
            ...
            for sem_rateio in range(2):
                if pd.isna(self.planilha.iloc[linha][f'CR-SR{sem_rateio+1}']): # Verifica se o campo está vazio
                    ...
                    acessar_iframe_default(self.navegador,self.tempo_espera) # Acessa Iframe de valor
                    opcoes_pagamento(self.navegador,'//*[@id="mul_dadosDaCobranca__dadosDoFaturamentoVariavel__dataVencimentoValorCobranca__formaDeEntradaDosRecursos_ori"]/option[1]','move_this_right_mul_dadosDaCobranca__dadosDoFaturamentoVariavel__dataVencimentoValorCobranca__formaDeEntradaDosRecursos')#Loop para selecionar as opções de pagamento
                    enviarkey_elemento(self.navegador,'var_dadosDaCobranca__dadosDoFaturamentoVariavel__dataVencimentoValorCobranca__data__',By.NAME,data_venc) # Envia data da cobrança
                    enviarkey_java(self.navegador,'var_dadosDaCobranca__dadosDoFaturamentoVariavel__dataVencimentoValorCobranca__valor__',self.planilha.iloc[linha][f'VALORSR{sem_rateio+1}']) # Envia Valor
                    clicar_elemento(self.navegador,'action.save',By.NAME) # Clica para salvar.
                    acessar_iframe_default(self.navegador,self.tempo_espera) # Acessa Iframe primario
                    clicar_elemento(self.navegador,'action.save',By.NAME) # Clica para salvar.
